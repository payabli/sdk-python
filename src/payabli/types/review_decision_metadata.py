# This file was auto-generated by Fern from our API Definition.

import typing

import pydantic
import typing_extensions
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ..core.serialization import FieldMetadata
from .bank_review_decision_reason import BankReviewDecisionReason


class ReviewDecisionMetadata(UniversalBaseModel):
    """
    Details of a reviewer's decision, when one has been made.
    """

    decline_reason: typing_extensions.Annotated[
        typing.Optional[BankReviewDecisionReason],
        FieldMetadata(alias="declineReason"),
        pydantic.Field(alias="declineReason", description="The decline reason, when the case was denied."),
    ] = None
    """
    The decline reason, when the case was denied.
    """

    note: typing.Optional[str] = pydantic.Field(default=None)
    """
    A free-text note attached to the decision.
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
